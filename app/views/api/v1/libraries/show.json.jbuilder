...
json.opening_hours {
	json.extract! @library.opening_hour, :mo, :tu, :we, :th, :fr, :sa, :su, :note
} if @library.opening_hour
json.projects {
	json.array!(@library.projects) do |project|
		json.extract! project, :code, :name
	end
}
json.services {
	json.array!(@library.services) do |service|
